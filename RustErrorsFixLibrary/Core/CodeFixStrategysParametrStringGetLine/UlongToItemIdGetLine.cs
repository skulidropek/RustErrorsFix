...
{
    internal class UlongToItemIdGetLine : CodeFixStrategyStringGetLine
    {
        public UlongToItemIdGetLine(CodeFixManager codeFixManager) : base(codeFixManager)
        {
        }
        ...
        public override string Fix(string code, (int, int) errorLine, string errorLineString)
        {
            ...
            try
            {
                ...
                code = code.Replace(errorLineString, errorLineString.Replace(item2, "new ItemId(" + item2.Replace(");", "") + ")"));
            }
            catch { }

            if (Regex.IsMatch(errorLineString, @"Convert.ToUInt64\(.+\)"))
                code = code.Replace(errorLineString, Regex.Replace(errorLineString, @"(Convert.ToUInt64\(.+\))", "new ItemId($1)"));
            //else
            //    code = code.Replace(errorLineString, Regex.Replace(errorLineString, @"\.uid\s=\s(.+);", ".uid = new ItemId($1)"));
