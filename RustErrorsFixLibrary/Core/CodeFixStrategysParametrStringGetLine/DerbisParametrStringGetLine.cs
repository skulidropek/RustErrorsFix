...
{
    internal class DerbisParametrStringGetLine : CodeFixStrategyParametrStringGetLine
    {
        public DerbisParametrStringGetLine(CodeFixManager codeFixManager) : base(codeFixManager)
        {
        }
        ...
        public override string Fix(string code, (int, int) errorLine, string errorLineString, GroupCollection groupCollection)
        {
            ...
            return code.Replace(lineText, lineText.Replace("Debris", "Physics_Debris"));//Regex.Replace(errorLineString, field + @"\s*==\s*([\d\w])+(\s|\)|,)", $"{field} == new {type}($1)$2"));
        }
    }
}
